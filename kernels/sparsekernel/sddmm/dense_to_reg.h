#ifndef THIRD_PARTY_SPARSEKERNEL_SDDMM_DENSE_TO_REG_H_
#define THIRD_PARTY_SPARSEKERNEL_SDDMM_DENSE_TO_REG_H_
...
#pragma unroll
    for (int x_item_idx = 0; x_item_idx < kColabItemsX_; ++x_item_idx) {
      ...
      for (int inner_idx = 0; inner_idx < kValuesPerLoad_; ++inner_idx) {
        ...
        // a staging register.
        if (nonzeros > 0) {
          Store(kDimK_ * sparsekernel::Load(column_indices_),
...
#pragma unroll
      for (int k_item_idx = 0; k_item_idx < kThreadItemsK_; ++k_item_idx) {
        ...
        Store(sparsekernel::Load(matrix), matrix_fragment_ + fragment_offset);
        matrix += kBlockWidth;
      }
...
#endif // THIRD_PARTY_SPARSEKERNEL_SDDMM_DENSE_TO_REG_H_
