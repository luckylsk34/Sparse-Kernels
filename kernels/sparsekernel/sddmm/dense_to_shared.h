#ifndef THIRD_PARTY_SPARSEKERNEL_SDDMM_DENSE_TO_SHARED_H_
#define THIRD_PARTY_SPARSEKERNEL_SDDMM_DENSE_TO_SHARED_H_
...
#pragma unroll
    for (int k_item_idx = 0; k_item_idx < kThreadItemsK_; ++k_item_idx) {
      ...
      Store(sparsekernel::Load(matrix_), matrix_fragment_ + k_item_idx);
...
#pragma unroll
    for (int k_item_idx = 0; k_item_idx < kThreadItemsK_; ++k_item_idx) {
      if (residue > 0) {
        // Load the values into smem.
        Store(sparsekernel::Load(matrix), matrix_fragment_ + k_item_idx);
      }
...
#endif // THIRD_PARTY_SPARSEKERNEL_SDDMM_DENSE_TO_SHARED_H_
