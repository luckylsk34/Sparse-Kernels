#ifndef THIRD_PARTY_SPARSEKERNEL_SDDMM_OUTPUT_TILE_H_
#define THIRD_PARTY_SPARSEKERNEL_SDDMM_OUTPUT_TILE_H_
...
template <int kBlockItemsX, int kBlockWidth> struct OutputTile {
  //
  /// Static members.
  //
...
#pragma unroll
    for (int x_item_idx = 0; x_item_idx < kThreadItemsX_; ++x_item_idx) {
      if (nonzeros > 0) {
        sparsekernel::Store(output_fragment_[x_item_idx], output_values_);
      }
...
#endif // THIRD_PARTY_SPARSEKERNEL_SDDMM_OUTPUT_TILE_H_
