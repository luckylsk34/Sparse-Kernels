#ifndef THIRD_PARTY_SPARSEKERNEL_SPMM_SPARSE_TILE_H_
#define THIRD_PARTY_SPARSEKERNEL_SPMM_SPARSE_TILE_H_
 ...
 */
template <typename Value, int kBlockItemsK, int kBlockWidth> struct SparseTile {
...
#pragma unroll
    for (int k_item_idx = 0; k_item_idx < kThreadItemsK_; ++k_item_idx) {
      ...
      Store(sparsekernel::Load(values_), values_tile);

      if (TypeUtils<ScalarValue>::IsMixed()) {
        // If we're using 16-bit indices, don't scale to avoid overflow.
        Store(sparsekernel::Load(column_idxs_), column_idxs_tile);
      } else {
        ...
        VectorCompute<Value>::Mul(
            rhs_columns_, sparsekernel::Load(column_idxs_), column_idxs_tile);
      }
...
#pragma unroll
  ...
  __device__ __forceinline__ void Residue(int residue) {
    ...
    const int kResidueUpdate =
        static_cast<int>(threadIdx.x) * kResidueUpdateStride;
...
#pragma unroll
    for (int k_item_idx = 0; k_item_idx < kScalarThreadItemsK; ++k_item_idx) {
      ...
      if (residue <= static_cast<int>(threadIdx.x))
        return;

      // Load the values into smem.
      Store(sparsekernel::Load(values), values_tile);

      if (TypeUtils<ScalarValue>::IsMixed()) {
        // If we're using 16-bit indices, don't scale to avoid overflow.
        Store(sparsekernel::Load(column_idxs), column_idxs_tile);
      } else {
        // Load the column indices into smem. Scale the index by the number
        // of columns in the rhs matrix for our lookups into the dense rhs.
        VectorCompute<ScalarValue>::Mul(
            rhs_columns_, sparsekernel::Load(column_idxs), column_idxs_tile);
      }
...
#endif // THIRD_PARTY_SPARSEKERNEL_SPMM_SPARSE_TILE_H_
