#ifndef THIRD_PARTY_SPARSEKERNEL_SPMM_DENSE_TILE_H_
#define THIRD_PARTY_SPARSEKERNEL_SPMM_DENSE_TILE_H_
...
#pragma unroll
        for (int x_item_idx = 0; x_item_idx < kThreadItemsX_; ++x_item_idx) {
          ...
          if (predicates_n.GetBit(x_item_idx)) {
            ...
            matrix_fragment_[fragment_offset] = sparsekernel::Load(matrix);
  ...
  ResidueLoadAndCompute(int residue, const Predicates &predicates_n,
                        const ScalarValue *lhs_tile, float *output_fragment) {
    const ScalarIndex *row_offsets = row_offsets_base_;

    // If we're only going to perform a single iteration of the inner loop,
    // pull the predicate check out of the loop.
    if ((kThreadItemsX_ == 1) && !predicates_n.GetBit(0))
      return;
          ...
          for (int x_item_idx = 0; x_item_idx < kThreadItemsX_; ++x_item_idx) {
            ...
            if ((kThreadItemsX_ == 1) || predicates_n.GetBit(x_item_idx)) {
              ...
              VectorCompute<Value>::FMA(
                  lhs_values[elt_idx], sparsekernel::Load(matrix),
...
#endif // THIRD_PARTY_SPARSEKERNEL_SPMM_DENSE_TILE_H_
