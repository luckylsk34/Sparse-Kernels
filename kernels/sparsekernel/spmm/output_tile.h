#ifndef THIRD_PARTY_SPARSEKERNEL_SPMM_OUTPUT_TILE_H_
#define THIRD_PARTY_SPARSEKERNEL_SPMM_OUTPUT_TILE_H_
 ...
 */
template <typename Value, int kBlockItemsX, int kBlockWidth> struct OutputTile {
  //
  /// Static members.
  //
    ...
    for (int x_item_idx = 0; x_item_idx < kThreadItemsX_; ++x_item_idx) {
      ...
      if (predicates_n.GetBit(x_item_idx)) {
        ...
        if (TypeUtils<Value>::IsMixed()) {
          ...
          sparsekernel::Store(out, output_matrix_);
        } else {
...
#endif // THIRD_PARTY_SPARSEKERNEL_SPMM_OUTPUT_TILE_H_
